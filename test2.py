...
class Dialog(QDialog, Ui_Dialog):
    def __init__(self, parent=None):
        ...
        self.distance_array2 = []  # 存储最短路径
        self.max_distance = 50  # 最大距离假设为10

        self.pushButton_2.clicked.connect(self.start_process)

        self.comboBox_3.currentIndexChanged.connect(self.apply_control_type())
        # 监听模式切换
        pi = np.pi
    ...
    def start_process(self):
        ...
        self.apply_control_type()



    # def update_comboBox_2_options(self, mode):
    #     if mode == "通信":
    #         self.comboBox_2.clear()
    #         self.comboBox_2.addItems(["选项1", "选项2"])
    #     elif mode == "成功率":
    #         self.comboBox_2.clear()
    #         self.comboBox_2.addItems(["独立选项1", "独立选项2"])

    def apply_control_type(self):
        if self.comboBox_3.currentText() == "通信":
            control_type = self.comboBox_2.currentText()
            print(f"控制类型 (通信模式): {control_type}")
            self.ax.clear()
            ...
            if control_type == "分级指控":
                ...
                self.update_function = self.mesh_greedy_routing  # Assuming this is your correct function
            elif control_type == "mesh指控（冗余路由）":
                self.update_function = self.mesh_redundant_routing  # Assuming this is your correct function
            else:
                self.update_function = None
            # 根据控制类型执行不同的操作
        elif self.comboBox_3.currentText() == "成功率":
            print("b模式的独立效果")
            # 执行b模式的独立效果

            self.update_success_rate_function()
    ...
    def update_success_rate_function(self):
        ...
        # 绘制折线图
        data1 = np.array(self.farthest_distances)
        data2 = np.array(self.hierarchical_distances)

        results1 = np.vectorize(self.success_rate_conclusion)(data1)
        results2 = np.vectorize(self.success_rate_conclusion)(data2)
        results1 = np.mean(results1)
        results2 = np.mean(results2)
    ...
    def success_rate_conclusion(self, length):  # 关于功率的计算
        if length == 0:
            ...
            return 36 / (36 + (length * np.sqrt(length) / (np.pi * 36)))
        else:
            return 36 / (36 + math.exp(length / 8 - 5) / (np.pi ** 2))
